...
class SpatialNeuralNetwork:
    ...
    def compute_unreachable_neurons(self) -> int:
        """
        Computes the number of neurons that cannot be reached from the output neurons.
        A lower number means better connectivity from outputs.

        Returns:
            int: Number of unreachable neurons (0 means all neurons are reachable)
        """
        with self.neuron_lock:
            ...
            unreachable = total_neurons - reachable_count
